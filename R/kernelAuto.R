...
#             time of measurement, and the remaining columns contain the       #
...
#             the measurement.                                                 #
#                                                                              #
# kType     : a character. One of "epan", "uniform", or "gauss". Specifies the #
#             form of the kernel function.                                     #
#                                                                              #
# lType     : a character. One of "identity", "log", "logistic". Sepcifies the #
#             form of the link function.                                       #
#                                                                              #
# times     : a numeric or vector object containing time points                #
#                                                                              #
# distanceFunction : a character object giving the distance function to be     #
#             used.                                                            #
#                                                                              #
# nCores    : a numeric. Number of cores to use for auto-tune implementation   #
#                                                                              #
#------------------------------------------------------------------------------#
#                                                                              #
# Returns a list. Values are for optimal bandwidth                             #
#                                                                              #
#   betaHat : matrix, row k contains parameter estimates for kth time point    #
#   sd      : matrix, row k contains standard deviations for kth time point    #
...
#   minMSE  : matrix, row k contains minimum standard error for kth time point #
#                                                                              #
#------------------------------------------------------------------------------#
kernelAuto <- function(data.x, 
  ...
  bw <- seq(from = 2*(quantile(x = range, probs = 0.75) - 
                      quantile(x = range, probs = 0.25))*nPatients^(-0.7),
            to =   2*(quantile(x = range, probs = 0.75) - 
                      quantile(x = range, probs = 0.25))*nPatients^(-0.3), 
            length = 50)
  ...
  mseFunc <- function(x) {
    ...
    tst <- x > 0.0
    if( sum(tst) == 0L ) stop("no positive values")
    x[!tst] <- NA
  ...
  opt_h <- apply(X = MSE, MARGIN = 2, FUN = mseFunc)
