...
distanceTI <- function(xIs, 
  ...
  for( i in 1L:nrow(data.y) ) {
    ...
    for( k in 1:lbw ) {
      mat[,k] <- local_kernel(t = kernelTime, h = bandwidth[k], kType = kType)
    }
...
distanceTD <- function(xIs, 
  ...
  if( !isTRUE(all.equal(lbw,(ncol(data.x)-2L))) ) bandwidth <- rep(bandwidth, ncol(data.x)-2L)
  ...
  funcXY <- function(x, dfx, dfy) {
    ...
    return(local_kernel(t = kernelTime, h = bandwidth[x], kType = kType))
  ...
  disXY <- apply(array(1L:lbw), 1, funcXY, dfx=data.x, dfy=data.y)
  
  disX <- disXY[1:nx,,drop=FALSE]
  disY <- disXY[(nx+1):(nx+ny),,drop=FALSE]
  
  dis <- lapply(1L:nrow(data.y), function(x){disY[x,]*disX[xIs[[x]]$v,,drop=FALSE]})
